    ...
    <section className="ums-section-gradient py-28">
      <div className="mx-auto max-w-[1320px] px-6 lg:px-8">
        <div className="mx-auto max-w-xl text-center">
          ...
          <h2 className="mt-3 text-[32px] font-bold text-ink sm:text-[38px]">
            <span className="ums-underline">Start Your Journey</span>
          </h2>
          <p className="mt-5 text-[15px] leading-[1.7] text-gray-500">
        ...
        <div className="mt-12 flex flex-wrap justify-center gap-x-10 gap-y-3 border-b border-[#e9f1fe]">
          {TABS.map((t, i) => (
            <button 
              ...
              type="button" 
              onClick={() => setActive(i)} 
              className={`-mb-px border-b-2 pb-4 text-[17px] transition-colors ${active === i ? "border-brandblue text-brandblue" : "border-transparent text-gray-500 hover:text-ink"}`}
            >
              {t}
            </button>
          ))}
          ...
          {COURSES.map((c) => (
            <article key={c.title} className="overflow-hidden rounded-[15px] bg-white shadow-sm ring-1 ring-slate-100">
              <div className="relative p-4">
                ...
                <span className={`absolute bottom-2 left-1/2 -translate-x-1/2 -rotate-[7deg] rounded-full ${c.tagBg} px-4 py-1.5 text-[15px] font-medium text-teal shadow-xs`}>
                  {c.tag}
                </span>
              </div>
        ...
        <div className="mx-auto mt-10 max-w-xl text-center">
          ...
          <Link href="#courses" className="mt-5 inline-flex items-center gap-2 text-[15px] font-medium text-brandblue hover:underline">
