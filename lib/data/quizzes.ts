// import { Quiz, QuizStatus, SortOption } from "../types/quiz";
...
import { Quiz, QuizStatus, SortOption } from "../types/quiz";
import { classrooms } from "./dashboardData";
// <-- fix this to wherever `classrooms` is actually exported from
...
const MOCK_QUIZZES: Quiz[] = [
  ...
  },
  {
    ...
    description:
      "Covers core UI/UX principles: layout, typography, color theory, and Figma workflows.",
    status: "draft",
...
export async function getQuizzes(params: GetQuizzesParams = {}): Promise<Quiz[]> {
  ...
  if (params.classroom && params.classroom !== "all") {
    results = results.filter((q) => q.classroom === params.classroom);
  }
...
export async function getClassrooms(): Promise<string[]> {
  // Sourced from the same `classrooms` list used on the dashboard/my-classroom
  // pages, so quiz filter options always match the teacher's real classes.
  const names = Array.from(new Set(classrooms.map((c) => c.title))).sort();
  return delay(names, 200);
}
