...
export default function QuizzesPage() {
  ...
  return (
    <div className="space-y-6 p-8">
      <div>
        <h2 className="text-2xl font-black tracking-tight text-indigo-950 dark:text-slate-100">Quizzes</h2>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center gap-2 py-16 text-center">
          <p className="text-lg font-semibold text-slate-700 dark:text-slate-200">No quizzes found</p>
          <p className="text-sm text-slate-500 dark:text-slate-400">No quizzes have been assigned to you yet.</p>
        </div>
      ) : (
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  <p className="text-sm font-bold text-indigo-950 dark:text-slate-100">{q.title}</p>
                ...
                {q.bestScore !== null ? (
                  <p className="mt-4 text-2xl font-black text-indigo-950 dark:text-slate-100">
                    {q.bestScore}
                  </p>
                ) : s === "open" ? (
                  <Link
                  ...
                  <button
                    disabled
                    className="mt-4 w-full cursor-not-allowed rounded-lg bg-slate-200 py-2 text-xs font-semibold text-slate-400 dark:bg-slate-800 dark:text-slate-500"
                  >
                    Not available
                  </button>
                )}
